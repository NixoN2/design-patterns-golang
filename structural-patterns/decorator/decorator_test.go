...
func TestPizzaDecorator_AddIngredient(t *testing.T) {
	...
	if !strings.Contains(pizzaResult, expectedText) {
		t.Errorf("When calling the add ingredient of the pizza decorator it must return the text %s, snot '%s'", pizzaResult,expectedText)
	}
}
...
func TestOnion_AddIngredient(t *testing.T) {
	...
	if err == nil {
		t.Errorf("When calling AddIngredient on the onion decorator without Ingredient field must return an error, not string with '%s'",onionResult)
	}
...
func TestMeat_AddIngredient(t *testing.T) {
	...
	if err == nil {
		t.Errorf("When calling AddIngredient on the meat decorator without Ingredient field must return an error, not string with '%s'",meatResult)
	}
